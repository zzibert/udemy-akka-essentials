...
class TimedAssertionsSpec extends TestKit(ActorSystem("TimeAssertionsSpec", ConfigFactory.load().getConfig("specialTimedAssertionsConfig")))
  with ImplicitSender
  with WordSpecLike
  ...
  "A worker actor" should {
    ...
    "reply with valid work at a reasonable cadence" in {
      within(1 second) {
        ...
        val results: Seq[Int] = receiveWhile[Int](max = 2 seconds, idle=50 millis, messages = 10) {
...
object TimedAssertionsSpec {
  ...
  class WorkerActor extends Actor {
    override def receive: Receive = {
      case "work" => // simulate long computation
        Thread.sleep(500)
